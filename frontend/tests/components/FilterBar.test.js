...
describe('FilterBar Component', () => {
  ...
  describe('Basic Filters', () => {
    it('should render status and priority select components in basic section', () => {
      const wrapper = mount(FilterBar, {
        global: { plugins: [vuetify] }
      })

      // Find the basic filters section (the first v-row after the header)
      const basicFiltersRow = wrapper.findAll('.v-row')[0] // First row contains basic filters
      const basicSelects = basicFiltersRow.findAll('[data-testid="v-select"]')

      // Should have exactly 2 selects in the basic section: status and priority
      expect(basicSelects).toHaveLength(2)

      // Verify the advanced section is hidden by default
  ...
  describe('Advanced Filters Toggle', () => {
    ...
    it('should render date range inputs in advanced section', async () => {
      const wrapper = mount(FilterBar, {
        global: { plugins: [vuetify] }
      })
      ...
      expect(textFields.length).toBeGreaterThanOrEqual(4) // 2 for created, 2 for completed
    })
  ...
  describe('Date Presets', () => {
    it('should render date preset buttons in advanced section', async () => {
      const wrapper = mount(FilterBar, {
        global: { plugins: [vuetify] }
      })
      ...
      expect(datePresetButtons.length).toBeGreaterThan(0)
    })
  ...
  describe('Filter Synchronization', () => {
    ...
    it('should update local filters when store filters change', async () => {
      // Create a new mock store with different initial values
      const newMockStore = {
        ...mockTaskStore,
        filters: {
          ...mockTaskStore.filters,
      ...
      useTaskStore.mockReturnValue(newMockStore)

      const wrapper = mount(FilterBar, {
        global: { plugins: [vuetify] }
      })

      // Since we're mocking, test that component initializes with store values
      expect(wrapper.vm.localFilters.status).toEqual(['completed'])
      expect(wrapper.vm.localFilters.priority).toEqual(['medium'])
    })
  })
